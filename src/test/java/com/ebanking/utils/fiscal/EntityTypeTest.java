...
class EntityTypeTest {
    ...
    void codeToEntityType() throws Exception {
        assertEquals(UNIPERSONAL, EntityType.codeConverter.from("20"));
        assertEquals(COMPANY, EntityType.codeConverter.from("30"));
    }

    // Test para código no válido
    @Test
    void codeNotMapped()  {
        assertThrows(Exception.class, () ->  EntityType.codeConverter.from("10"));
    }

    // Test para verificar otros códigos cercanos en el mapa
    @Test
    void codeToEntityTypeBoundaries() throws Exception {
        assertEquals(UNIPERSONAL, EntityType.codeConverter.from("27"));
        assertEquals(COMPANY, EntityType.codeConverter.from("50"));
    }

    // Test para verificar código fuera del rango (fuera del mapa)
    @Test
    void codeToEntityTypeOutOfRange()  {
        assertThrows(Exception.class, () ->  EntityType.codeConverter.from("1101"));
    }
    ...
    void invalidCode() {
        var code = "abc";
        Exception exception = assertThrows(Exception.class, () -> EntityType.codeConverter.from(code));
    ...
    void codeWithExtraDigits() throws Exception {
        assertEquals(UNIPERSONAL, EntityType.codeConverter.from("201234"));
        assertEquals(COMPANY, EntityType.codeConverter.from("301234"));
    }
    ...
    void validCodeWithLeadingZeros() throws Exception {
        assertEquals(UNIPERSONAL, EntityType.codeConverter.from("200"));
        assertEquals(COMPANY, EntityType.codeConverter.from("300"));
    }
